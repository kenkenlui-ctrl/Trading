#!/usr/bin/env python3
"""Futu kline bootstrap for 7/31 HK (only 2 days back, well within futu window).

For each HK code missing 7/31 record, fetch futu kline for 7/31 close +
use latest cached snapshot for non-price fields. Save stub to daily_report.
"""
import json
import sqlite3
import sys
...
def main():
    ...
    print(f"Missing HK 7/31: {len(missing)}", flush=True)
    if not missing:
    ...
    for i, code in enumerate(missing, 1):
        try:
            ...
            snap["source"] = "futu-history-bootstrap-7-31"
