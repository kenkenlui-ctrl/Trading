...
def main():
    ...
    # Only backfill the audit window (6/26 - 7/15).
    # 7/15 was imported from static HTML on 2026-07-17, so it also
    # needs the rule re-applied (originally came as raw op).
    # 7/16+ records will be generated by the daily pipeline.
    rows = con.execute("""
        ...
        FROM daily_report
        WHERE report_date >= '2026-06-26' AND report_date <= '2026-07-15'
        ORDER BY report_date, code
    """).fetchall()
    print(f"Backfilling {len(rows)} records (2026-06-26 to 2026-07-15)")
