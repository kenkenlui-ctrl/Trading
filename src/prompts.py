...
- 唔可以寫「回調至支持區吸納做多博反彈至 MA20」呢類冇數字嘅空泛描述 — 要寫「回調至 $385-$392 區間（即今日低位 + 略低於 MA20 5%）吸納做多」。
...
【⚠️ 輸出完整性規則】
- summary + reasoning 必須係完整句子，唔可以截斷喺中間（例如「股價偏離 MA20 達 11.3」之後就要寫完結論）。
- 唔可以重複同一句字（例如「反彈 +1.40% 後反彈 +1.40%」係 bug，要避免）。
- 如果即將到達 token 上限，優先保證 summary 完整，reasons 可以稍短，但唔可以半句截斷。
...
- momentum_score: 純價格動能。MA20 > MA50 > MA100 +10；RSI 50-70 健康；今日 +2% 以上加分；MA200 以下但短炒 setup 都俾分（day-trade 唔睇 long-term trend）。
- order_flow_score: 純資金流。量比 > 1.5 大單流入加分；今日成交 > ADV 20d 加分；買賣盤力度（bid/ask imbalance）；北水/異常成交加分；TRUMP/VIX 等宏觀 risk-off 減分。
...
- momentum_score: pure price energy. MA20 > MA50 > MA100 +10; RSI 50-70 healthy; today > +2% = boost; below MA200 still gets points for short setups.
- order_flow_score: pure money flow. vol ratio > 1.5 + big-order inflow = boost; today's volume > ADV 20d = boost; bid/ask imbalance; northbound / abnormal turnover = boost; macro risk-off (VIX spike, Trump policy shocks) = penalty.
